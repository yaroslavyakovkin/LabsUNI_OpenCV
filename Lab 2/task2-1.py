...
и = 4
img = cv2.imread('2-3.jpg', flags=cv2.IMREAD_GRAYSCALE)
# cv2.namedWindow('edit', flags=cv2.WINDOW_NORMAL)

# thres ,edit = cv2.threshold(img, 50, 255, cv2.THRESH_BINARY) #Хорошо для 2-0
...
# edit = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 41, 20) # Хорошо для 2-4
...
for k in range(100, 255):
    thres ,edit = cv2.threshold(img, k, 255, cv2.THRESH_BINARY)
