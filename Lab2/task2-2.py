...
while True:
    ret, frame = cam.read()
    frame = cv2.resize(frame, (640, 360), cv2.INTER_LANCZOS4)
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    l, frame2 = cv2.threshold(frame, 90, 255, cv2.THRESH_BINARY)
    two_of_us = np.concatenate((frame, frame2), axis=0)
    two_of_us = np.concatenate((two_of_us, two_of_us), axis=1)
    cv2.imshow('two_of_us', two_of_us)
    if cv2.waitKey(1) == ord('q'):
        break